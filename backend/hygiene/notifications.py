...
class WhatsAppNotifier:
    WHATSAPP_TOKEN = getattr(settings, 'WHATSAPP_TOKEN', '') or ''
    PHONE_NUMBER_ID = getattr(settings, 'WHATSAPP_PHONE_NUMBER_ID', '') or '785662747963620'
    BASE_URL = f"https://graph.facebook.com/v22.0/{PHONE_NUMBER_ID}/messages"

    @classmethod
    def send_request(cls, payload):
        token = getattr(settings, 'WHATSAPP_TOKEN', '') or cls.WHATSAPP_TOKEN
        phone_id = getattr(settings, 'WHATSAPP_PHONE_NUMBER_ID', '') or cls.PHONE_NUMBER_ID
        url = f"https://graph.facebook.com/v22.0/{phone_id}/messages"
        
        if not token:
            logger.warning("WHATSAPP_TOKEN not configured; skipping WhatsApp notification.")
            return None
